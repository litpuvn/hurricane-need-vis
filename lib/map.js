...
function display_graphs_for_region(region_id, region_name) {
  needsByHours(23);
  clean_wordCloud();
  ...
  let concerns = all_districts.features[region_id].properties.concern.toString().split(',');
  let userContent = all_districts.features[region_id].properties.user_content;
  let userId = all_districts.features[region_id].properties.userId;
  console.log(userId)
  let tweet = all_districts.features[region_id].properties.tweet;
  let timestamp = all_districts.features[region_id].properties.timestamp;
...
display_graphs_for_region(29, 'Downtown')
