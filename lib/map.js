...
function highlightFeature(e) {
  ...
  display_graphs_for_region(region_id, region_name, concerns);
}


function display_graphs_for_region(region_id, region_name, concerns ) {
  clean_wordCloud();
  ...
  let all_districts = window.districts;
  // Extract other contents from region id.


    let userContent = all_districts.features[dist_num].properties.user_content;
    let userId = all_districts.features[dist_num].properties.userId;
    let tweet = all_districts.features[dist_num].properties.tweet;
    let timestamp = all_districts.features[dist_num].properties.timestamp;


  // let userContent = layer_properties.user_content;

  drawWordCloud(userContent);  // show Region tweet
  showCanvas(region_name, region_id);

   // userId = layer_properties.userId;
   // tweet = layer_properties.tweet;
   // timestamp = layer_properties.timestamp;
